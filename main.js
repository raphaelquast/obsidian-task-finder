...
let defaultQueryFolders = {
	"GeoSphere/Projects/CGLOPS/": "@u +-2w #CGLOPS ",
	"GeoSphere/Projects/HSAF/": "@u +-2w #HSAF ",
	"GeoSphere/Projects/GeoSphere/": "@u +-2w #GeoSphere ",
}
...
class TaskSuggester extends obsidian.SuggestModal {
  ...
  getSuggestions(query){
	...
	const datematch = query.match(/(?:\s|^)(?:@)(\d{8}|\d{4}|\d{2})?(?:\s|$)/)
	...
	if ( datematch ) {
		...
		if ( date ) {
			if (date.length == 8) {
				...
				now = moment(`${now.format('YYYYMM')}${date}`)
			} 	
		}
	...
	const tagmatch = query.match(/(?:\s|^)#(\S*)/g)
	if ( tagmatch ) {
		for (i in tagmatch) {
